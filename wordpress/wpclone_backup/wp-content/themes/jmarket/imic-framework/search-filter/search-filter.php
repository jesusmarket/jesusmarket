...
* Set up Plugin Globals
*/
if ( ! class_exists( 'SearchAndFilterIN' ) )
{
	class SearchAndFilterIN
	{
		...
		{
			wp_enqueue_style( 'searchandfilter', IMIC_THEME_PATH . '/imic-framework/search-filter/style.css', false, 1.0, 'all' );
		}
		...
		*/
		...
		public function get_search_filter_form($submitlabel, $search_placeholder, $fields, $types, $labels, $hierarchical, $hide_empty, $show_count, $post_types, $order_by, $order_dir, $operators, $all_items_labels, $empty_search_url, $add_search_param, $class)
		{
			...
			$template_url = imic_get_template_url('template-sermons.php');
			$template_perma = get_permalink($template_url);
			$returnvar .= '
				<form action="'. $template_perma .'" method="post" class="searchandfilter'.$addclass.'">
					<div>';
					...
					$returnvar .= '
						...
						$returnvar .=
							...
							<input class="btn btn-default" type="submit" value="'.$submitlabel.'">
						</li>';
		...
		*/
		...
		{
			$returnvar = '<select multiple="form-control multiple" name="'.$args['sf_name'].'[]" class="postform">';
			$returnvar .= walk_taxonomyIN('multiselect', $args);
			$returnvar .= "</select>";
		...
		{
			$returnvar = '<ul>';
			$returnvar .= walk_taxonomyIN('checkbox', $args);
			$returnvar .= "</ul>";
		...
		public function generate_wp_radio($args, $name, $currentid = 0, $labels = null, $defaultval = "0")
		{
			...
			$returnvar .= walk_taxonomyIN('radio', $args);
			$returnvar .= "</ul>";
		...
		public function generate_select($dropdata, $name, $currentid = 0, $labels = null, $defaultval = "0")
		{
			...
			$returnvar .= '<select class="form-control postform" name="'.SF_FPRE.$name.'">';
			if(isset($labels))
			{
				if($labels->all_items!="")
...
function walk_taxonomyIN( $type = "checkbox", $args = array() ) {
...
if ( class_exists( 'SearchAndFilterIN' ) )
{
	global $SearchAndFilterIN;
	$SearchAndFilterIN	= new SearchAndFilterIN();
}
...
// classes
require_once(SEARCHANDFILTER_THEME_DIR."/imic-framework/search-filter/of-list-table.php");
require_once(SEARCHANDFILTER_THEME_DIR."/imic-framework/search-filter/of-taxonomy-walker.php");

// admin screens & plugin mods
//require_once(SEARCHANDFILTER_THEME_DIR."/imic-framework/search-filter/of-admin.php");
